...
void save_config(char *mname)
{
    ...
    vector<ident *> ids;
    enumerate(idents, ident, id, (ids.emplace_back(&id),ids.back()));
    ids.sortname();
    ...
    for( size_t i = 0; i < ents.size(); ++i ) // extended
    {
        ...
        if(e.type!=ET_EMPTY || forcesave)
        {
            ...
            if(entities::maylink(e.type))
            {
                ...
                if(verbose >= 2) conoutf("entity %s (%zu) saved %zu links", entities::findname(e.type), i, links.size());
            }
            count++;
        }
    }
        ...
        for( size_t i = 0; i < ents.size(); ++i )
        {
            ...
            if((maptype == MAP_OCTA || (maptype == MAP_MAPZ && hdr.version <= 29)) && ents[i]->type == ET_LIGHTFX && ents[i]->attrs[0] == LFX_SPOTLIGHT)
            {
                ...
                if(( 0 <= closest && closest < ents.size() ) && closedist <= 100)
                {
                    ...
                    if(verbose) conoutf("\frWARNING: auto linked spotlight %zu to light %d", i, closest);
                }
            }
        }
